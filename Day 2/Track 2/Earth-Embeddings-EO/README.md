...
- Precomputed embedding pool hosted on the Hugging Face Hub at [`zterrabyte/eurosat-gitrsclip-embeddings`](https://huggingface.co/datasets/zterrabyte/eurosat-gitrsclip-embeddings) (7.7 MB, float16, L2-normalized). Notebooks 3 and 4 pull it at workshop time with `hf_hub_download` (a few seconds on Colab).
